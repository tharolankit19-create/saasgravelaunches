...
-- ─────────────────────────────────────────────────────────────
--  SPOTLIGHT BIDS — the no-login "pay to rank" auction board.
--
--  Anyone drops a SaaS link and names a bid; the highest live bid sits at #1.
--  Each paid bid is good for 24 hours. No account required (user_id is
--  optional, attached only if someone happens to be signed in). Additive and
--  self-contained; RLS stays off — every read/write goes through the service
--  role, and the only public key to a row is its opaque token.
-- ─────────────────────────────────────────────────────────────
