    ...
    <footer className="mt-16 border-t border-ink-900/8 bg-paper-100">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-ink-900/8 pt-6 text-[12px] text-ink-400">
          ...
          <p>
            Part of{" "}
            <a href={SAASGRAVE} className="text-ink-700 hover:text-violet-600">
              Saasgrave
            </a>
            .
          </p>
      ...
      <ul className="space-y-2">
        {links.map((l) => (
          <li key={l.href + l.label}>
            {l.external ? (
              <a href={l.href} className="text-[13px] text-ink-500 transition hover:text-violet-600">
                {l.label}
              </a>
            ) : (
              <Link href={l.href} className="text-[13px] text-ink-500 transition hover:text-violet-600">
                {l.label}
              </Link>
            )}
          </li>
